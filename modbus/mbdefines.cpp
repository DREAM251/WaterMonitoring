...
static int endian = get_system_endian();
...
//按照MODBUS的格式将UINT16分解成两个UINT8类型
unsigned char u16split(unsigned short v, int part)
{
    I82I16 t;t.I16 = v;
    if(endian == __BIG_ENDIAN){
        if(part == 0)
            return t.I8[0];
        else
            return t.I8[1];
...
{
    if(endian == __LITTLE_ENDIAN)
        return ((unsigned short)highbyte<<8 | (unsigned short)lowbyte);
    else
        return ((unsigned short)lowbyte<<8 | (unsigned short)highbyte);
}
